                ...
                getSettingsPanel() {
                    return Settings.SettingPanel.build(_ => this.saveAndUpdate(), ...[
                        ...
                        new Settings.SettingGroup("Emoji").append(
                            ...
                            new Settings.Slider("Rozmiar", "Zalecany rozmiar emoji to 40px.", 16, 64, this.settings.emojiSize, size=>this.settings.emojiSize = size, {markers:[16,20,32,40,64], stickToMarkers:true})
                        ),
						new Settings.SettingGroup("Avatar Profilu").append(...[
							new Settings.Switch("AVATAR CUSTOM", "Włączenie lub wyłączenie clientsided profile avatar.", this.settings.clientsidePfp, value => this.settings.clientsidePfp = value),
							new Settings.Textbox("URL", "Bezpośredni adres URL do awatara profilu, który chcesz (PNG, JPG lub GIF; zalecany jest obraz kwadratowy).", this.settings.pfpUrl,
								image => {
									try {
										new URL(image)
									} catch {
										return Toasts.error('To jest nieprawidłowy adres URL!')
									}
									this.settings.pfpUrl = image
								}
							)
						]),
						new Settings.SettingGroup("Banner Profilu").append(...[
                                new Settings.Switch("BANNER CUSTOM", "Włącz lub wyłącz baner profilu klienta.", this.settings.clientsideBanner, value => this.settings.clientsideBanner = value),
                                new Settings.Textbox("URL", "Bezpośredni adres URL do baneru profilowego, który chcesz (PNG, JPG lub GIF; zalecany jest rozmiar 600x240).", this.settings.bannerUrl,
                                    image => {
                                        try {
                                            new URL(image)
                                        } catch {
